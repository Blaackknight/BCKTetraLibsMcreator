...
public abstract class ModElementGUI<GE extends GeneratableElement> extends ViewBase implements IHelpContext {
    ...
    private net.mcreator.ui.modgui.ModElementGUI.ModElementCreatedListener<GE> modElementCreatedListener;
    ...
    public void setModElementCreatedListener(net.mcreator.ui.modgui.ModElementGUI.ModElementCreatedListener<GE> modElementCreatedListener) {
    ...
    protected final void finalizeGUI() {
        ...
        if (pages.size() > 1) {
            ...
            save.addActionListener(event -> {
                java.util.List<ValidationGroup> errors = new ArrayList<>();
    ...
    private void disableUnsupportedFields() {
        java.util.List<String> exclusions = mcreator.getGeneratorConfiguration()
                .getUnsupportedDefinitionFields(modElement.getType());

        java.util.List<String> inclusions = mcreator.getGeneratorConfiguration()
        ...
        if (exclusions != null && inclusions != null) { // can't exclude and include together
            ...
            Map<Container, java.util.List<Component>> includedComponents = new HashMap<>();
    ...
    private void finishModCreation(boolean closeTab) {
        ...
        if (this.tabIn != null && closeTab)
            ...
            mcreator.getTabs().getTabs().stream().filter(e -> e.getContent() == this)
                    .forEach(e -> e.setIcon(((net.mcreator.ui.modgui.ModElementGUI<?>) e.getContent()).getViewIcon()));
    }
