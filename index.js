...
async function load() {
    let datas = {
        ...
        procedures: { count: 0, list: [], ftl: [], ftl_count: 0 },
        variables: { count: 0, list: [] },
    ...
    // Lire "procedures"
    try {
        let files = await fs.readdir("./procedures/");
        if (files.length > 0) {
            files.forEach((f) => {
                if (f.startsWith("$")) {
                    datas.categories.list.push(f);
                    datas.categories.count++;
                } else {
                    datas.procedures.list.push(f);
                    datas.procedures.count++;
                }
            });
        }
    } catch (err) {
        console.error("Erreur lecture procedures:", err);
    }

    // Lire "variables"
    try {
        let files = await fs.readdir("./variables/");
        if (files.length > 0) {
            files.forEach((f) => {
                datas.variables.list.push(f);
                datas.variables.count++;
            });
        }
    } catch (err) {
        console.error("Erreur lecture variables:", err);
    }

    // Lire "triggers"
    try {
        let files = await fs.readdir("./triggers/");
        if (files.length > 0) {
            files.forEach((f) => {
                datas.triggers.list.push(f);
                datas.triggers.count++;
            });
        }
    } catch (err) {
        console.error("Erreur lecture triggers:", err);
    }

    // Lire "templates"
    try {
        let files = await fs.readdir("./templates/");
        if (files.length > 0) {
            files.forEach(async (f) => {
                let ff = await fs.readdir(`./templates/${f}`);
                if (ff.length > 0) {
                    ff.forEach((e) => {
                        if(!datas.templates[f]) datas.templates[f] = [];
                        datas.templates[f].push(e);
                        if(!datas.templates[`${f}_count`]) datas.templates[`${f}_count`] = 0
                        datas.templates[`${f}_count`]++;
                    })
                }
            });
        }
    ...
    // Lire "procedures"
    try {
        let files = await fs.readdir("./forge-1.20.1/procedures/");
        if (files.length > 0) {
            files.forEach((f) => {
                datas.procedures.ftl.push(f);
                datas.procedures.ftl_count++;
            });
        }
    } catch (err) {
        console.error("Erreur lecture procedures:", err);
    ...
    // Maintenant qu'on a fini toutes les lectures, on peut loguer
    console.log(datas);

    // Copie filtrée de la liste .json (sans extension)
    const prList = datas.procedures.list.map((filename) =>
        filename.replace(/\.json$/, "")
    );

    // Copie filtrée de la liste .java.ftl (sans extension)
    const ftlList = datas.procedures.ftl.map((filename) =>
        filename.replace(/\.java\.ftl$/, "")
    );

    // Maintenant, on crée les sets à partir des copies
    const listSet = new Set(prList);
    const ftlSet = new Set(ftlList);

    // Calcul des éléments présents dans l'un et pas dans l'autre
    const onlyInList = [...listSet].filter(x => !ftlSet.has(x));
    const onlyInFtl = [...ftlSet].filter(x => !listSet.has(x));

    // Comparaison
    if (onlyInList.length === 0 && onlyInFtl.length === 0) {
        console.log("Les noms (sans extension) sont identiques.");
    } else {
        console.log("Il existe des différences :");
        if (onlyInList.length > 0) {
            console.log("En .json mais pas en .java.ftl :", onlyInList);
        }
        if (onlyInFtl.length > 0) {
            console.log("En .java.ftl mais pas en .json :", onlyInFtl);
        }
    }
}
